...
GEN = PROGRAM_DIR / "generated_data"
...
def main() -> None:
    ...
    if not args.skip_profiles:
        ...
        if args.overwrite_profiles:
            argv += ["--overwrite"]
        run_step("Step 1 - generate_profiles", PROGRAM_DIR / "generate_profiles.py", argv, args.dry_run)
    else:
    ...
    if not args.skip_build:
        ...
        run_step("Step 2a - build_situations_from_external", PROGRAM_DIR / "build_situations_from_external.py",
                 argv, args.dry_run)
    else:
    ...
    if not args.skip_training:
        ...
        if args.profile_indices:
            argv += ["--profile-indices"] + [str(i) for i in args.profile_indices]
        run_step("Step 3 - generate_training_data", PROGRAM_DIR / "generate_training_data.py", argv, args.dry_run)
    else:
